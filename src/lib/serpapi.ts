...
export async function searchShopping(query: string, category: string): Promise<NormalizedProduct[]> {
  ...
  return items.map(i => normalizeShoppingResult(i, category)).filter((p): p is NormalizedProduct => p !== null);
}
