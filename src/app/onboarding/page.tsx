...
export default function Onboarding() {
  ...
  if (step === 'form') return (
    <div className="p-6 flex flex-col gap-4">
      ...
      <button disabled={!name || phone.length < 10} onClick={() => setStep('otp')} className="bg-black text-white rounded-2xl py-4 disabled:opacity-40">Получить код</button>
    </div>
  );
  ...
  if (step === 'otp') return (
    <div className="p-6 flex flex-col gap-4">
      ...
      <p className="text-sm text-zinc-500">Мы отправили SMS-код на {phone}</p>
      ...
      <button onClick={() => code === (process.env.NEXT_PUBLIC_DEMO_OTP ?? '000000') ? setStep('interests') : setErr('Неверный код')} className="bg-black text-white rounded-2xl py-4">Подтвердить</button>
